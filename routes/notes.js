...
router.get('/', async (req, res, next) => {
  try {
    const ownerId = new ObjectId(req.session.user._id.toString()); // 👈 force string first
    const tagsParam = req.query.tags || '';
    const tags = tagsParam.split(',').map(s => s.trim()).filter(Boolean);
    const projectId = req.query.projectId ? new ObjectId(req.query.projectId.toString()) : undefined;
